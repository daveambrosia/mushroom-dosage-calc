...
class ADC_Shortcode {
    ...
    public static function render($atts) {
        ...
        ), $atts, 'dosage_calculator');
        
        // Convert string booleans
        $bool_keys = array('show_edibles', 'show_mushrooms', 'show_quick_converter', 
                          'show_compound_breakdown', 'show_safety_warning', 'allow_custom', 'allow_submit');
        ...
        $url_tab = isset($_GET['t']) ? sanitize_text_field($_GET['t']) : '';
        if ($url_tab === 'e' && $atts['show_edibles']) {
            $initial_tab = 'edibles';
        } elseif ($url_tab === 'm' && $atts['show_mushrooms']) {
            $initial_tab = 'mushrooms';
        } else {
            $initial_tab = $atts['default_tab'];
        }
        $is_edibles = ($initial_tab === 'edibles');
        ...
        wp_localize_script('adc-calculator', 'adcConfig', array(
        ...
        $safety_items    = $safety_is_html ? [] : array_filter(array_map('trim', explode("\n", $safety_raw)));
        ...
        ob_start();
        ?>
        <div class="adc-calculator<?php echo $is_edibles ? ' adc-tab-edibles' : ''; ?>" id="adc-calculator" data-template="<?php echo esc_attr($template); ?>"<?php echo $theme_attr; ?>>
            ...
            <div class="adc-input-section" id="adc-input-section" tabindex="-1">
                ...
                <div class="adc-input-group adc-box adc-mushroom-input" id="adc-content-mushrooms" role="tabpanel" aria-labelledby="adc-tab-mushrooms" data-collapsible data-section="strain" data-collapse-mode="select-only">
                    ...
                    <?php echo self::compound_inputs('strain'); ?>
                </div>
                <?php endif; ?>
                ...
                <div class="adc-input-group adc-box adc-edible-input" id="adc-content-edibles" role="tabpanel" aria-labelledby="adc-tab-edibles" data-collapsible data-section="edible-product" data-collapse-mode="select-only">
                    ...
                    <?php echo self::compound_inputs('edible'); ?>
                </div>
                <?php endif; ?>
            </div>
            ...
            <footer class="adc-footer">
                ...
                <p class="adc-version">v<?php echo ADC_VERSION; ?></p>
            </footer>
            
            <?php echo self::strain_modal(); ?>
            <?php echo self::edible_modal(); ?>
    ...
    private static function build_tolerance_options() {
        $labels = array(28 => '28+ days (No tolerance)', 21 => '21 days (3 weeks)', 
                       14 => '14 days (2 weeks)', 7 => '7 days (1 week)', 1 => '1 day (yesterday)');
        $options = '';
        for ($d = 28; $d >= 1; $d--) {
            $label = isset($labels[$d]) ? $labels[$d] : "{$d} days";
            $selected = ($d === 28) ? ' selected' : '';
    ...
    private static function compound_inputs($type) {
        ...
        $unit = ($type === 'edible') ? 'mcg/piece' : '';
        $wrapper_id = "adc-custom-{$type}-wrapper";
        
        $unit_label = ($type === 'edible') ? '(total mcg for package)' : '(mcg per gram)'; $html = "<div class=\"adc-custom-wrapper\" id=\"{$wrapper_id}\" style=\"display:none;\"><p class=\"adc-custom-unit-label\">{$unit_label}</p><div class=\"adc-custom-grid\">";
        foreach ($compounds as $c) {
            $label = ucfirst($c);
            $placeholder = ($c === 'psilocybin') ? '5000' : '0';
...
HTML;
    ...
    private static function edible_modal() {
        ...
        $fields = '';
        foreach ($compounds as $id => $label) {
            $placeholder = ($id === 'psilocybin') ? '25000' : '0';
