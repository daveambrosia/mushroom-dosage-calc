...
class ADC_REST_API {
	...
	public static function register_routes() {
		...
		register_rest_route(
			$namespace,
			'/admin/submissions/(?P<id>\d+)/approve',
			array(
				'methods'             => 'POST',
		...
		register_rest_route(
			$namespace,
			'/admin/submissions/(?P<id>\d+)/reject',
			array(
				'methods'             => 'POST',
	...
	public static function submit_custom( $request ) {
		...
		// Honeypot: bots fill the website field; humans leave it empty (BUG-004).
		if ( ! empty( $body['website'] ) ) {
			return new WP_Error( 'spam_detected', 'Invalid submission', array( 'status' => 400 ) );
		}

		// Validate
		if ( empty( $body['type'] ) || ! in_array( $body['type'], array( 'strain', 'edible' ), true ) ) {
		...
		// Rate limiting - Two-tier approach (transient + DB)
		$ip              = self::get_client_ip();
		$rate_limit_pass = self::check_rate_limit( $ip );

		if ( is_wp_error( $rate_limit_pass ) ) {
			return $rate_limit_pass;
		}

		// Secondary check: DB-based rate limit (5 per hour)
		global $wpdb;
		...
		$result = ADC_Submissions::create(
			array(
				...
				'ip_address'      => $ip,
				'user_agent'      => sanitize_text_field( substr( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ?? '' ), 0, 500 ) ), // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			)
		);
	...
	public static function admin_get_submissions( $request ) {
		...
		$submissions = ADC_Submissions::get_all(
			array(
				'status' => $status,
				'type'   => $type,
			)
	...
	private static function send_file_response( $data, $filename, $content_type ) {
		...
		$response->header( 'Content-Disposition', 'attachment; filename="' . $filename . '"' );
		$response->header( 'Content-Length', (string) strlen( $json ) );
		$response->header( 'Cache-Control', 'no-cache, no-store, must-revalidate' );
		$response->header( 'Pragma', 'no-cache' );
		$response->header( 'Expires', '0' );
		return $response;
	...
	/**
	 * Check transient-based rate limiting (fast, in-memory check).
	 *
	 * @since 2.19.0
	...
	private static function check_rate_limit( $ip ) {
		...
		if ( $attempts >= 10 ) {
			// Exceeded burst limit (10 requests per 15 minutes)
			return new WP_Error(
				'rate_limit_burst',
