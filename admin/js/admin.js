...
(function ($) {
	...
	$( document ).on(
		...
		function (e) {
			...
			confirmAction(
				'Delete "' + name + '"?\n\nThis will permanently remove this strain from your database.',
				function () {
	...
	$( document ).on(
		...
		function (e) {
			...
			confirmAction(
				'Delete "' + name + '"?\n\nThis will permanently remove this edible from your database.',
				function () {
					$row.addClass( 'deleting' );
					$link.text( 'Deleting...' );
	...
	function initClickableCards() {
		$( '.adc-dashboard-cards .adc-card' ).each(
			function () {
				var $card = $( this );
...
function adcShowDetailsModal(sub) {
	...
	if (data.brand) {
		html += "<div class='adc-detail-row'><span class='adc-detail-label'>Brand:</span> " + escapeHtml( data.brand ) + "</div>";
	}
	...
	if (sub.submitter_notes) {
		html += "<div class='adc-detail-row'><span class='adc-detail-label'>Notes:</span> " + escapeHtml( sub.submitter_notes ) + "</div>";
	}
	html += "</div></div>";
