...
class ADC_Sheets_Importer {
    ...
    public static function save_settings($settings) {
        ...
        $clean = array(
            ...
            'import_mode'    => in_array($settings['import_mode'] ?? '', array('add_new', 'update', 'replace'))
                                ? $settings['import_mode'] : 'update',
            'auto_sync'      => !empty($settings['auto_sync']),
            'sync_frequency' => in_array($settings['sync_frequency'] ?? '', array('hourly', 'twicedaily', 'daily', 'weekly'))
    ...
    public static function map_headers($headers, $type = 'strain') {
        ...
        foreach (self::$header_mappings as $db_col => $aliases) {
            ...
            if ($type === 'strain' && in_array($db_col, array('brand', 'product_type', 'pieces_per_package', 'total_mg'))) {
                continue;
            }
    ...
    private static function process_row($row, $column_map, $type, $mode) {
        ...
        if (!empty($data['product_type'])) {
            ...
            $plural_map = array(
                'gummies' => 'gummy', 'capsules' => 'capsule', 'chocolates' => 'chocolate',
                'tinctures' => 'tincture', 'teas' => 'tea',
            );
        ...
        if (isset($data['is_active'])) {
            ...
            $data['is_active'] = in_array($val, array('1', 'yes', 'true', 'active', 'y')) ? 1 : 0;
        } else {
            $data['is_active'] = 1;
        }
        ...
        if (empty($data['short_code'])) {
            $data['short_code'] = ADC_DB::generate_short_code($type === 'edible' ? 'edible' : 'strain', $data['name'] ?? 'Unknown');
        }
    ...
    private static function find_by_short_code($type, $short_code) {
        global $wpdb;
        $table = ADC_DB::table($type === 'edible' ? 'edibles' : 'strains');
