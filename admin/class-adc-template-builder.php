...
class ADC_Template_Builder {
    ...
    private static $variable_groups = array(
        'Core Colors' => array(
            'bg'          => array('type' => 'color', 'label' => 'Background', 'desc' => 'Main calculator background'),
            'text'        => array('type' => 'color', 'label' => 'Text', 'desc' => 'Primary text color'),
            'accent'      => array('type' => 'color', 'label' => 'Accent', 'desc' => 'Links, highlights, active states'),
            'surface'     => array('type' => 'color', 'label' => 'Surface', 'desc' => 'Card and panel backgrounds'),
            'surface-alt' => array('type' => 'color', 'label' => 'Secondary Background', 'desc' => 'Alternate card/panel background (e.g. zebra rows, secondary sections)'),
            'border'      => array('type' => 'color', 'label' => 'Border Color', 'desc' => 'General border color'),
        ),
        'Header & Tabs' => array(
            'header-bg'       => array('type' => 'color', 'label' => 'Header Background'),
            'header-text'     => array('type' => 'color', 'label' => 'Header Text'),
            'header-border'   => array('type' => 'text',  'label' => 'Header Border', 'desc' => 'Border below the calculator header. e.g. \'none\' or \'1px solid #ddd\'. Leave blank to use the global Border Color.'),
            'tab-bg'          => array('type' => 'color', 'label' => 'Tab Background'),
            'tab-text'        => array('type' => 'color', 'label' => 'Inactive Tab Text', 'desc' => 'Text color for unselected tabs'),
            'tab-active-bg'   => array('type' => 'color', 'label' => 'Active Tab Background'),
            'tab-active-text' => array('type' => 'color', 'label' => 'Active Tab Text'),
            'tab-hover-bg'    => array('type' => 'color', 'label' => 'Tab Hover Background'),
        ),
        'Body & Inputs' => array(
            'body-bg'        => array('type' => 'color', 'label' => 'Body Background'),
            'input-bg'       => array('type' => 'color', 'label' => 'Input Background'),
            'input-border'   => array('type' => 'color', 'label' => 'Input Border'),
            'input-focus-bg' => array('type' => 'color', 'label' => 'Input Focus Background'),
            'focus-ring'     => array('type' => 'text', 'label' => 'Selection Outline', 'desc' => 'Outline shown when a form field is focused. e.g. \'none\' or \'0 0 0 3px rgba(59,130,246,0.5)\''),
        ),
        'Buttons' => array(
            'btn-bg'          => array('type' => 'color', 'label' => 'Button Background'),
            'btn-text'        => array('type' => 'color', 'label' => 'Button Text'),
            'btn-border'      => array('type' => 'color', 'label' => 'Button Border'),
            'btn-hover-bg'    => array('type' => 'color', 'label' => 'Button Hover Background'),
            'btn-primary-bg'  => array('type' => 'color', 'label' => 'Primary Button Background'),
            'btn-primary-text' => array('type' => 'color', 'label' => 'Primary Button Text'),
        ),
        'Special Areas' => array(
            'unit-active-bg'   => array('type' => 'color', 'label' => 'Unit Toggle Active Background'),
            'unit-active-text' => array('type' => 'color', 'label' => 'Unit Toggle Active Text'),
            'converter-bg'     => array('type' => 'color', 'label' => 'Result Box Background', 'desc' => 'Background of the unit converter / result display area'),
            'safety-bg'        => array('type' => 'color', 'label' => 'Safety Warning Background'),
            'safety-border'    => array('type' => 'color', 'label' => 'Safety Warning Border'),
            'modal-header-bg'  => array('type' => 'color', 'label' => 'Popup Header Background'),
            'modal-header-text' => array('type' => 'color', 'label' => 'Popup Header Text'),
        ),
        'Layout' => array(
            'border-width'     => array('type' => 'text', 'label' => 'Border Width', 'desc' => 'e.g. "1px", "2px", "3px"'),
            'radius'           => array('type' => 'text', 'label' => 'Border Radius', 'desc' => 'e.g. "0px", "8px", "16px"'),
            'shadow'           => array('type' => 'text', 'label' => 'Shadow (Global)', 'desc' => 'Default shadow for all elements. e.g. "none" or "0 4px 24px rgba(0,0,0,0.1)"'),
            'header-shadow'    => array('type' => 'text', 'label' => 'Header Shadow', 'desc' => 'Overrides global shadow for the header. e.g. "none" or "0 2px 8px rgba(0,0,0,0.15)"'),
            'box-shadow'       => array('type' => 'text', 'label' => 'Panel Shadow', 'desc' => 'Shadow for input boxes and panels. e.g. \'none\' or \'0 2px 8px rgba(0,0,0,0.1)\''),
            'card-shadow'      => array('type' => 'text', 'label' => 'Card Shadow', 'desc' => 'Overrides global shadow for dose result cards. e.g. "none"'),
            'container-border' => array('type' => 'text', 'label' => 'Container Border', 'desc' => 'Border around the entire calculator. e.g. \'2px solid #000\' or \'none\''),
        ),
        'Fonts' => array(
            'font-heading' => array('type' => 'text', 'label' => 'Heading Font', 'desc' => "e.g. 'Inter', sans-serif or 'Space Mono', monospace"),
            'font-body'    => array('type' => 'text', 'label' => 'Body Font', 'desc' => "e.g. 'Work Sans', sans-serif or 'Georgia', serif"),
        ),
    );
    ...
    public function ajax_set_active_template() {
        ...
        wp_send_json_success(array('message' => __('Template activated!', 'ambrosia-dosage-calc'), 'slug' => $slug));
    }
    ...
    public function handle_early_actions() {
        ...
        if ($page !== 'dosage-calculator-template-builder') {
            return;
        }
        ...
        if ($action === 'duplicate' && isset($_GET['slug'])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($post_action === 'export' && isset($_POST['slug'])) {
                $this->handle_export();
            }
            if ($post_action === 'import_template') {
                $this->handle_import();
            }
    ...
    public static function get_builtin_templates() {
        if (self::$builtin_templates !== null) {
    ...
    public function render_page() {
        if (!current_user_can('manage_options')) {
            wp_die(__('Unauthorized', 'ambrosia-dosage-calc'));
        }
        ...
        echo '<h1>🎨 ' . esc_html__('Template Builder', 'ambrosia-dosage-calc') . '</h1>';
    ...
    private function handle_save() {
        ...
        if (empty($name)) {
            add_settings_error('adc_template_builder', 'empty_name', __('Template name is required.', 'ambrosia-dosage-calc'), 'error');
            return;
        }
        ...
        foreach ($all_keys as $key) {
            ...
            $value = isset($_POST[$post_key]) ? trim(wp_unslash($_POST[$post_key])) : '';
        ...
        wp_redirect(admin_url('admin.php?page=dosage-calculator-template-builder&saved=1'));
        exit;
    }
    ...
    private function handle_delete() {
        ...
        $templates = self::get_custom_templates();
        $templates = array_filter($templates, function($t) use ($slug) {
            return $t['slug'] !== $slug;
        });

        self::save_custom_templates($templates);

        // If this template was the active one, reset to brutal
        $settings = ADC_DB::get_settings();
        if (($settings['template'] ?? '') === $slug) {
            $settings['template'] = 'brutal';
            ADC_DB::update_settings($settings);
        }

        wp_redirect(admin_url('admin.php?page=dosage-calculator-template-builder&deleted=1'));
        exit;
    }
    ...
    private function handle_duplicate() {
        ...
        if (!$original_template) {
            wp_die(__('Original template not found.', 'ambrosia-dosage-calc'));
        }
        ...
        wp_redirect(admin_url('admin.php?page=dosage-calculator-template-builder&action=edit&slug=' . $new_slug . '&duplicated=1'));
        exit;
    }
    ...
    private function handle_import() {
        ...
        if (!isset($_POST['adc_import_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['adc_import_nonce'])), 'adc_import_template')) {
            add_settings_error('adc_template_builder', 'nonce_fail', __('Security check failed. Please try again.', 'ambrosia-dosage-calc'), 'error');
            return;
        }
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            add_settings_error('adc_template_builder', 'upload_error', sprintf(__('File upload error: %s', 'ambrosia-dosage-calc'), $file['error']), 'error');
            return;
        }
        ...
        if (!isset($imported_data['adc_template']) || (int)$imported_data['adc_template'] !== 1) {
        ...
        foreach ($imported_data['variables'] as $key => $value) {
            if (!in_array($key, $all_known_keys)) {
        ...
        wp_redirect(admin_url('admin.php?page=dosage-calculator-template-builder&imported=1'));
        exit;
    }
    ...
    private function handle_export() {
        ...
        $template = self::get_custom_template($slug);
        if (!$template) {
            wp_die(__('Template not found.', 'ambrosia-dosage-calc'));
        }
    ...
    private function render_list_view() {
        $templates = self::get_custom_templates();
        $current_template = ADC_DB::get_setting('template', 'brutal');
        ...
        wp_localize_script('adc-template-builder', 'adcTemplateBuilderData', array(
        ...
        foreach ($templates as $t) {
            ...
            $is_active = ($current_template === $t['slug']);
            ...
            // Name
            echo '<td><strong><a href="' . esc_url($edit_url) . '">' . $name . '</a></strong>';
            ...
            foreach ($swatch_colors as $sk => $color) {
                echo '<span style="display:inline-block;width:18px;height:18px;background:' . esc_attr($color) . ';border:1px solid #ccc;border-radius:2px;" title="' . esc_attr($sk) . '"></span>';
            }
            echo '</span>';
            echo '</td>';
            ...
            // Actions
            echo '<td class="adc-tb-actions" data-slug="' . $slug . '">';
            ...
            if (!$is_active) {
                echo '<button class="button button-small adc-set-active" data-slug="' . $slug . '" data-nonce="' . esc_attr(wp_create_nonce('adc_set_active_template')) . '">' . esc_html__('Set Active', 'ambrosia-dosage-calc') . '</button> ';
            }
        ...
        foreach ($builtins as $slug => $bt) {
            ...
            echo '<h3 class="adc-builtin-card-title">' . $name . '</h3>';
    ...
    private function render_edit_view() {
        ...
        wp_localize_script('adc-template-builder', 'adcTemplateBuilderData', array(
            ...
            'startFromSlug'    => isset($_GET['start_from']) ? sanitize_key($_GET['start_from']) : '',
            'currentAction'    => isset($_GET['action']) ? sanitize_key($_GET['action']) : '',
        ));

        $back_url = admin_url('admin.php?page=dosage-calculator-template-builder');
        echo '<p><a href="' . esc_url($back_url) . '">&larr; ' . esc_html__('Back to Templates', 'ambrosia-dosage-calc') . '</a></p>';
        ...
        // Top save button
        echo '<p class="submit" style="margin:0 0 10px; padding:0;">';
        ...
        echo '<tr><th><label for="template_name">' . esc_html__('Template Name', 'ambrosia-dosage-calc') . '</label></th>';
        echo '<td><input type="text" id="template_name" name="template_name" value="' . esc_attr($name) . '" class="regular-text" required>';
        echo '<p class="description">' . esc_html__('Give your template a unique name.', 'ambrosia-dosage-calc') . '</p></td></tr>';

        // Start from...
        echo '<tr><th><label for="adc_start_from">' . esc_html__('Start From...', 'ambrosia-dosage-calc') . '</label></th>';
        echo '<td><select id="adc_start_from" onchange="adcLoadBuiltinTemplate(this.value)">';
        echo '<option value="">' . esc_html__('-- Blank (set your own values) --', 'ambrosia-dosage-calc') . '</option>';
        foreach ($builtins as $slug => $bt) {
            echo '<option value="' . esc_attr($slug) . '">' . esc_html($bt['name']) . '</option>';
        }
        echo '</select>';
        echo '<p class="description">' . esc_html__('Load all values from a built-in template as a starting point.', 'ambrosia-dosage-calc') . '</p></td></tr>';
        ...
        foreach (self::$variable_groups as $group_label => $group_vars) {
            ...
            $is_open = ($tier === 1); // Only Tier 1 is open by default

            // Advanced divider before Tier 3
            if ($tier === 3 && $last_tier < 3) {
            ...
            if ($has_text_vars) {
                ...
                foreach ($group_vars as $key => $def) {
                    ...
                    if ($key === 'border-width') {
                        ...
                        echo '<input type="range" min="0" max="5" step="0.5" value="' . esc_attr($num_val) . '" class="adc-range-slider" data-target="' . esc_attr($post_key) . '" data-unit="px">';
                        ...
                        $num_val = floatval(preg_replace('/[^0-9.]/', '', $current_val));
                        echo '<div class="adc-range-control">';
                        echo '<input type="range" min="0" max="24" step="1" value="' . esc_attr($num_val) . '" class="adc-range-slider" data-target="' . esc_attr($post_key) . '" data-unit="px">';
                        ...
                        echo '</select>';
                        echo '<input type="text" id="' . esc_attr($post_key) . '" name="' . esc_attr($post_key) . '" value="' . esc_attr($current_val) . '" class="regular-text adc-tb-text-input adc-shadow-custom-input" data-key="' . esc_attr($key) . '" style="' . ($select_val !== 'custom' ? 'display:none;margin-top:6px;' : 'margin-top:6px;') . '" placeholder="' . esc_attr__('Custom CSS shadow value', 'ambrosia-dosage-calc') . '">';
                        ...
                        foreach ($font_options as $fval => $flabel) {
                            $font_selected = ($current_val === $fval) ? ' selected' : '';
                            echo '<option value="' . esc_attr($fval) . '"' . $font_selected . '>' . esc_html($flabel) . '</option>';
                        }
                        echo '</select>';
                    ...
                    <div class="adc-picker-grid">
                        ...
                        <div class="adc-picker-card" data-slug="<?php echo esc_attr($slug); ?>" tabindex="0" role="button" aria-label="<?php echo esc_attr(sprintf(__('Start from %s template', 'ambrosia-dosage-calc'), $bt['name'])); ?>">
                            ...
                            ">
                                ...
                                <div style="background:<?php echo $header; ?>;color:<?php echo $text; ?>;padding:6px 8px;text-align:center;font-weight:700;font-size:10px;">🍄 Calculator</div>
                                <div style="display:flex;background:<?php echo esc_attr($vars['tab-bg'] ?? '#f0f0f0'); ?>">
                                    <div style="flex:1;text-align:center;padding:3px;background:<?php echo $tabact; ?>;color:<?php echo esc_attr($vars['tab-active-text'] ?? '#fff'); ?>;font-size:8px;">Mushrooms</div>
                                ...
                                <div style="padding:6px;background:<?php echo esc_attr($vars['body-bg'] ?? $bg); ?>">
                                    <div style="background:<?php echo $surface; ?>;border-radius:4px;padding:4px 6px;margin-bottom:4px;">
                                    ...
                                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:3px;margin-bottom:4px;">
                                        <div style="background:<?php echo $surface; ?>;border-radius:4px;padding:3px;text-align:center;">
                                            <div style="font-size:7px;opacity:0.6;">Micro</div>
                                            <div style="color:<?php echo $accent; ?>;font-weight:700;">0.15g</div>
                                        </div>
                                        <div style="background:<?php echo $surface; ?>;border-radius:4px;padding:3px;text-align:center;outline:1px solid <?php echo $accent; ?>">
                                            <div style="font-size:7px;opacity:0.6;">Medium</div>
                                            <div style="color:<?php echo $accent; ?>;font-weight:700;">1.5g</div>
                                        </div>
                                    </div>
    ...
    private function render_preview_panel() {
        $preview_url = home_url('/adc-preview/');
        ?>
    ...
    private static function sanitize_css_value($value) {
        ...
        $v = str_replace(["\x00", "\x1F"], '', $v);
        // Block CSS expression() injection entirely
        if (stripos($v, 'expression(') !== false) return '';
