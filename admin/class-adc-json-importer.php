...
class ADC_JSON_Importer {
    ...
    public static function render_page() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adc_json_import_nonce'])) {
            if (!wp_verify_nonce($_POST['adc_json_import_nonce'], 'adc_json_import')) {
                    ...
                    <div class="notice <?php echo $notice_class; ?>">
                        ...
                        <ul>
                            <?php if (isset($result['strains'])): ?>
                                <li>Strains: <?php echo intval($result['strains']); ?> imported<?php 
                                    if (!empty($result['strains_skipped'])): 
                                        ?>, <?php echo intval($result['strains_skipped']); ?> skipped<?php 
                                    endif; 
                                ?></li>
                            <?php endif; ?>
                            <?php if (isset($result['edibles'])): ?>
                                <li>Edibles: <?php echo intval($result['edibles']); ?> imported<?php 
                                    if (!empty($result['edibles_skipped'])): 
                                        ?>, <?php echo intval($result['edibles_skipped']); ?> skipped<?php 
                                    endif; 
                                ?></li>
                            <?php endif; ?>
    ...
    private static function handle_upload() {
        if (!isset($_FILES['json_file']) || $_FILES['json_file']['error'] !== UPLOAD_ERR_OK) {
            ...
            $error_code = $_FILES['json_file']['error'] ?? UPLOAD_ERR_NO_FILE;
        ...
        $filename = sanitize_file_name( $_FILES['json_file']['name'] );
        ...
        if ( $_FILES['json_file']['size'] > 5 * 1024 * 1024 ) {
        ...
        if (isset($_POST['import_strains']) && !empty($strains_data)) {
            ...
            foreach ($strains_data as $index => $strain) {
                ...
                if ($existing && $duplicate_handling === 'skip') {
                    $skipped++;
                    continue;
                }
                ...
                if ($existing && $duplicate_handling === 'update') {
        ...
        if (isset($_POST['import_settings']) && !empty($settings_data)) {
            ...
            if ($gsheets_data) {
                $sanitized_gsheets = array(
                    ...
                    'import_mode'    => in_array($gsheets_data['import_mode'] ?? '', ['update', 'replace']) ? $gsheets_data['import_mode'] : 'update',
    ...
    private static function sanitize_settings($data) {
        ...
        // Boolean fields
        $bool_fields = array(
            'show_edibles', 'show_mushrooms', 'show_quick_converter',
            'show_compound_breakdown', 'show_safety_warning',
            'allow_custom', 'allow_submit', 'auto_submit_unknown_qr'
        );
    ...
    private static function find_by_code($code, $type) {
        ...
        global $wpdb;
        $table = $type === 'strain' ? ADC_DB::table('strains') : ADC_DB::table('edibles');
