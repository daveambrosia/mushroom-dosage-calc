...
class ADC_Sheets_Admin_Page {
    ...
    public static function register_routes() {
        ...
        register_rest_route($namespace, '/sheets/preview', array(
            'methods'             => 'POST',
        ...
        register_rest_route($namespace, '/sheets/import', array(
            'methods'             => 'POST',
    ...
    public static function rest_preview($request) {
        ...
        if (empty($url)) {
            return new WP_REST_Response(array('success' => false, 'message' => 'URL is required.'), 400);
        }

        $preview = ADC_Google_Sheets::preview($url, 5, $gid ?: null);
        if (is_wp_error($preview)) {
            return new WP_REST_Response(array('success' => false, 'message' => $preview->get_error_message()), 400);
        }
        ...
        return new WP_REST_Response(array('success' => true, 'data' => $preview));
    }
    ...
    public static function rest_import($request) {
        ...
        if (empty($url) || empty($type)) {
            return new WP_REST_Response(array('success' => false, 'message' => 'URL and type are required.'), 400);
        }

        if (!in_array($type, array('strain', 'edible'))) {
            return new WP_REST_Response(array('success' => false, 'message' => 'Type must be strain or edible.'), 400);
        }
    ...
    public static function enqueue_scripts($hook) {
        ...
        wp_enqueue_script(
            ...
            plugins_url('admin/js/google-sheets-admin.js', dirname(__FILE__)),
            array('jquery'),
            ADC_VERSION,
            true
        );

        wp_localize_script('adc-google-sheets-admin', 'adcSheetsAdmin', array(
