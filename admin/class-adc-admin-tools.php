...
class ADC_Admin_Tools {
	...
	public function render_taxonomies() {
		...
		$tab      = isset( $_GET['tab'] ) ? sanitize_key( wp_unslash( $_GET['tab'] ) ) : 'categories';
		$base_url = admin_url( 'admin.php?page=dosage-calculator-taxonomies' );
		?>
		<div class="wrap">
			<h1>Taxonomies</h1>
			<nav class="nav-tab-wrapper" style="margin-bottom:20px">
				<a href="<?php echo esc_url( $base_url . '&tab=categories' ); ?>" class="nav-tab <?php echo 'categories' === $tab ? 'nav-tab-active' : ''; ?>">Categories</a>
				<a href="<?php echo esc_url( $base_url . '&tab=product-types' ); ?>" class="nav-tab <?php echo 'product-types' === $tab ? 'nav-tab-active' : ''; ?>">Product Types</a>
			</nav>
		</div>
		<?php
		if ( 'product-types' === $tab ) {
	...
	public function render_import_export() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only display filter.
		$tab      = isset( $_GET['tab'] ) ? sanitize_key( wp_unslash( $_GET['tab'] ) ) : 'google-sheets';
		$base_url = admin_url( 'admin.php?page=dosage-calculator-import-export' );
		?>
		<div class="wrap">
			<h1>Import / Export</h1>
			<nav class="nav-tab-wrapper" style="margin-bottom:20px">
				<a href="<?php echo esc_url( $base_url . '&tab=google-sheets' ); ?>" class="nav-tab <?php echo 'google-sheets' === $tab ? 'nav-tab-active' : ''; ?>">Google Sheets</a>
				<a href="<?php echo esc_url( $base_url . '&tab=csv' ); ?>" class="nav-tab <?php echo 'csv' === $tab ? 'nav-tab-active' : ''; ?>">CSV</a>
				<a href="<?php echo esc_url( $base_url . '&tab=json' ); ?>" class="nav-tab <?php echo 'json' === $tab ? 'nav-tab-active' : ''; ?>">JSON</a>
				<a href="<?php echo esc_url( $base_url . '&tab=export' ); ?>" class="nav-tab <?php echo 'export' === $tab ? 'nav-tab-active' : ''; ?>">Export</a>
			</nav>
		</div>
		<?php
		switch ( $tab ) {
			...
			<div class="adc-card adc-card-narrow">
				...
				<table class="form-table">
					<tr>
						<th>Strains</th>
						<td>
							<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=adc_export&type=strains&format=csv&_wpnonce=' . wp_create_nonce( 'adc_export' ) ) ); ?>" class="button">Export CSV</a>
							<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=adc_export&type=strains&format=json&_wpnonce=' . wp_create_nonce( 'adc_export' ) ) ); ?>" class="button">Export JSON</a>
							<p class="description">
							<?php
							global $wpdb;
							echo (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . ADC_DB::table( 'strains' ) );
							?>
							strains</p>
						</td>
					</tr>
					<tr>
						<th>Edibles</th>
						<td>
							<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=adc_export&type=edibles&format=csv&_wpnonce=' . wp_create_nonce( 'adc_export' ) ) ); ?>" class="button">Export CSV</a>
							<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=adc_export&type=edibles&format=json&_wpnonce=' . wp_create_nonce( 'adc_export' ) ) ); ?>" class="button">Export JSON</a>
					...
					<tr>
						<th>Settings</th>
						<td>
							<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=adc_export&type=settings&format=json&_wpnonce=' . wp_create_nonce( 'adc_export' ) ) ); ?>" class="button">Export JSON</a>
					...
					<tr>
						<th>All Data</th>
						<td>
							<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=adc_export&type=all&format=json&_wpnonce=' . wp_create_nonce( 'adc_export' ) ) ); ?>" class="button button-primary">Export Everything (JSON)</a>
	...
	public function handle_export() {
		...
		$data     = array();
		$errors   = array();
		$filename = 'adc-export-' . $type . '-' . wp_date( 'Y-m-d' );
		...
		switch ( $type ) {
			case 'strains':
				$raw = ADC_Strains::get_all( array( 'active_only' => false ) );
				$check_db_error( $raw, 'Strains export' );
				$data = $raw ? self::format_strains_for_export( $raw ) : array();
				break;
			case 'edibles':
			...
			case 'settings':
				...
				$data['gsheets'] = get_option( 'adc_gsheets_settings', array() );
				$format          = 'json'; // Settings only as JSON
				break;
			case 'all':
				$raw_strains = ADC_Strains::get_all( array( 'active_only' => false ) );
				$check_db_error( $raw_strains, 'Strains' );

				$raw_edibles = ADC_Edibles::get_all( array( 'active_only' => false ) );
				$check_db_error( $raw_edibles, 'Edibles' );

				$data   = array(
					'strains'     => $raw_strains ? self::format_strains_for_export( $raw_strains ) : array(),
					'edibles'     => $raw_edibles ? self::format_edibles_for_export( $raw_edibles ) : array(),
					'settings'    => array_merge( get_option( 'adc_settings', array() ), array( 'gsheets' => get_option( 'adc_gsheets_settings', array() ) ) ),
		...
		if ( 'csv' === $format && in_array( $type, array( 'strains', 'edibles' ), true ) ) {
			...
			if ( false === $json ) {
				...
				echo $json; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- JSON export endpoint, already encoded.
			}
		}
	...
	public function render_product_types() {
		...
		if ( isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['adc_product_type_nonce'] ) ) {
			if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['adc_product_type_nonce'] ) ), 'adc_save_product_type' ) ) {
				wp_die( 'Security check failed' );
			}

			$action = isset( $_POST['pt_action'] ) ? sanitize_key( wp_unslash( $_POST['pt_action'] ) ) : 'add';

			if ( 'add' === $action && ! empty( $_POST['pt_name'] ) ) {
			...
			} elseif ( 'delete' === $action && ! empty( $_POST['pt_id'] ) ) {
	...
	public function render_categories() {
		...
		if ( isset( $_POST['cat_action'] ) && isset( $_POST['adc_category_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['adc_category_nonce'] ) ), 'adc_save_category' ) ) {
			$action = sanitize_text_field( wp_unslash( $_POST['cat_action'] ) );

			if ( 'add' === $action && ! empty( $_POST['cat_name'] ) ) {
				$name       = sanitize_text_field( wp_unslash( $_POST['cat_name'] ) );
				$slug       = isset( $_POST['cat_slug'] ) ? sanitize_key( wp_unslash( $_POST['cat_slug'] ) ) : sanitize_title( $name );
				$type       = isset( $_POST['cat_type'] ) ? sanitize_text_field( wp_unslash( $_POST['cat_type'] ) ) : 'both';
			...
			} elseif ( 'edit' === $action && ! empty( $_POST['cat_id'] ) ) {
				$wpdb->update(
					$table,
					array(
						'name'       => sanitize_text_field( wp_unslash( $_POST['cat_name'] ) ),
						'slug'       => isset( $_POST['cat_slug'] ) ? sanitize_key( wp_unslash( $_POST['cat_slug'] ) ) : '',
						'type'       => isset( $_POST['cat_type'] ) ? sanitize_text_field( wp_unslash( $_POST['cat_type'] ) ) : 'both',
						'sort_order' => isset( $_POST['cat_sort_order'] ) ? intval( $_POST['cat_sort_order'] ) : 0,
					),
			...
			} elseif ( 'delete' === $action && ! empty( $_POST['cat_id'] ) ) {
