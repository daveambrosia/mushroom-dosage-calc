...
class ADC_CSV_Importer {
    ...
    public static function render_page() {
        ...
        // Handle file upload
        $result = null;
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adc_import_nonce'])) {
            if (!wp_verify_nonce($_POST['adc_import_nonce'], 'adc_csv_import')) {
...
Penis Envy,12000,3000,600,PE-2024-001,high-potency</pre>
    ...
    private static function handle_upload() {
        ...
        $default_category = sanitize_key($_POST['default_category']);
        $duplicate_handling = sanitize_key($_POST['skip_duplicates']);
        ...
        if ($type === 'strain' && !isset($column_map['psilocybin'])) {
            fclose($handle);
            return new WP_Error('missing_field', 'Strain CSV must contain a "psilocybin" column.');
        }
        
        if ($type === 'edible' && !isset($column_map['pieces_per_package'])) {
            fclose($handle);
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            if ($type === 'edible' && empty($data['total_mg'])) {
    ...
    private static function map_headers($headers) {
        ...
        foreach ($headers as $index => $header) {
            ...
            foreach (self::$header_mappings as $field => $variants) {
                if (isset($map[$field])) continue; // Don't overwrite existing mappings
    ...
    private static function map_row($row, $column_map, $headers) {
        ...
        $fields = array(
            'name', 'batch_number', 'category', 
            'psilocybin', 'psilocin', 'norpsilocin', 'baeocystin', 'norbaeocystin', 'aeruginascin',
            'brand', 'product_type', 'pieces_per_package', 'total_mg', 'notes'
        );
        ...
        foreach ($fields as $field) {
            if (isset($column_map[$field]) && isset($row[$column_map[$field]])) {
                ...
                if (in_array($field, array('psilocybin', 'psilocin', 'norpsilocin', 'baeocystin', 'norbaeocystin', 'aeruginascin', 'pieces_per_package', 'total_mg'))) {
