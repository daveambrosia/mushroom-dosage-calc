...
class ADC_Admin_Edibles {
				...
				<tbody>
					...
					<?php else : ?>
						...
						foreach ( $edibles as $edible ) :
							...
							$pkg_total     = $mcg_per_piece * $pieces;
							?>
							<tr>
								...
								<td data-label="mcg/piece"><span class="adc-mobile-label">mcg/piece: </span><?php echo number_format( $mcg_per_piece ); ?></td>
								<td data-label="Pieces"><span class="adc-mobile-label">Pieces: </span><?php echo intval( $edible['pieces_per_package'] ); ?></td>
								<td data-label="Pkg Total"><span class="adc-mobile-label">Pkg Total: </span><strong><?php echo number_format( $pkg_total ); ?></strong></td>
								...
								<td data-label="Actions" class="adc-actions-cell">
									...
									<a href="#" class="button button-small adc-delete-edible adc-btn-delete" data-id="<?php echo esc_attr( $edible['id'] ); ?>" data-name="<?php echo esc_attr( $edible['name'] ); ?>">Delete</a>
								</td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
	...
	public function render_edible_form() {
		...
		if ( isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['adc_edible_nonce'] ) ) {
			if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['adc_edible_nonce'] ) ), 'adc_save_edible' ) ) {
				wp_die( 'Security check failed' );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'You do not have permission to manage edibles.', 'ambrosia-dosage-calc' ) );
			}
			...
			$data = array(
				...
				'pieces_per_package' => isset( $_POST['pieces_per_package'] ) ? max( 1, absint( $_POST['pieces_per_package'] ) ) : 1,
		...
		<div class="wrap adc-admin">
			...
			<form method="post">
				...
				<p class="description">Enter compound amounts in <strong>mcg (micrograms) PER PIECE</strong>. The calculator multiplies by pieces to show package totals.</p>
				<table class="form-table">
					<tr>
				...
				<h3>Compound Amounts (mcg per piece)</h3>
				<table class="form-table">
					<tr>
						<th><label for="psilocybin">Psilocybin <span class="required">*</span></label></th>
						<td><input type="number" id="psilocybin" name="psilocybin" value="<?php echo isset( $edible['id'] ) ? esc_attr( $edible['psilocybin'] ) : ''; ?>" min="0" placeholder="0"> mcg/piece</td>
					</tr>
					<tr>
						<th><label for="psilocin">Psilocin</label></th>
						<td><input type="number" id="psilocin" name="psilocin" value="<?php echo isset( $edible['id'] ) ? esc_attr( $edible['psilocin'] ) : ''; ?>" min="0" placeholder="0"> mcg/piece</td>
					</tr>
					<tr>
						<th><label for="norpsilocin">Norpsilocin</label></th>
						<td><input type="number" id="norpsilocin" name="norpsilocin" value="<?php echo isset( $edible['id'] ) ? esc_attr( $edible['norpsilocin'] ) : ''; ?>" min="0" placeholder="0"> mcg/piece</td>
					</tr>
					<tr>
						<th><label for="baeocystin">Baeocystin</label></th>
						<td><input type="number" id="baeocystin" name="baeocystin" value="<?php echo isset( $edible['id'] ) ? esc_attr( $edible['baeocystin'] ) : ''; ?>" min="0" placeholder="0"> mcg/piece</td>
					</tr>
					<tr>
						<th><label for="norbaeocystin">Norbaeocystin</label></th>
						<td><input type="number" id="norbaeocystin" name="norbaeocystin" value="<?php echo isset( $edible['id'] ) ? esc_attr( $edible['norbaeocystin'] ) : ''; ?>" min="0" placeholder="0"> mcg/piece</td>
					</tr>
					<tr>
						<th><label for="aeruginascin">Aeruginascin</label></th>
						<td><input type="number" id="aeruginascin" name="aeruginascin" value="<?php echo isset( $edible['id'] ) ? esc_attr( $edible['aeruginascin'] ) : ''; ?>" min="0" placeholder="0"> mcg/piece</td>
					</tr>
					<tr>
						<th>Package Total</th>
						<td>
							<strong id="total_mcg_pkg" class="adc-total-display">
							<?php
								$pieces    = max( 1, $edible['pieces_per_package'] ?? 1 );
								$total_mcg = ( ( $edible['psilocybin'] ?? 0 ) + ( $edible['psilocin'] ?? 0 ) ) * $pieces;
								echo number_format( $total_mcg );
							?>
							</strong> mcg
							<p class="description">( Psilocybin + Psilocin ) × pieces per package.</p>
						</td>
					</tr>
				</table>
